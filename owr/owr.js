...
var Duties, Game, Kinks, OWRMain, Roll, StartSelector, Twists, Xmoney, Ymoney, button, calcMoney, d, d19, div, form, h1, label, li, option, p, ref, rollCounter, select, small, table, tbody, td, th, tr, ul,
  slice = [].slice;
...
ref = React.DOM, button = ref.button, div = ref.div, form = ref.form, h1 = ref.h1, label = ref.label, li = ref.li, option = ref.option, p = ref.p, select = ref.select, small = ref.small, table = ref.table, tbody = ref.tbody, td = ref.td, th = ref.th, tr = ref.tr, ul = ref.ul;
...
Game = React.createClass({displayName: "Game",
  ...
  render: function() {
    ...
    return div({}, div({
      ...
      className: 'col-xs-2'
    }, 'Money')), _.map(this.state.rolls, (function(_this) {
      return function(roll, i) {
        ...
        return tr({
          key: roll.key
        }, td(classes, _this.listify(roll.duty())), td(classes, _this.listify(roll.kink())), td(classes, _this.listify(roll.twist())), td(classes, "$" + (roll.money()) + " " + (roll.debug())));
      };
    })(this))))));
  }
});
